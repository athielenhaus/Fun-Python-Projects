...
def countdown(count):
    ...
    if count > 0:
        timer_countdown = window.after(1000, countdown, count-1)
    else:
        start_timer()
        if reps % 2 == 0:
            checks += "✓"
...
start_bttn = Button(text= "Start", width= 5, command= start_timer)
start_bttn.grid(column=0, row=3)

reset_bttn = Button(text= "Reset", width= 5, command= reset_timer)
reset_bttn.grid(column=2, row=3)

timer = Label(text="Timer", fg=GREEN, bg= YELLOW, font= (FONT_NAME, 35, "bold"))
timer.grid(column=1, row= 0)

checkmarks = Label(fg=GREEN, bg= YELLOW, font= (14))
